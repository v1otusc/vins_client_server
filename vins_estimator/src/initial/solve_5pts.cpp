...
/**
 * @brief 通过分解本质矩阵得到 R t
 *        - findFundamentalMat() 采用 RANSAC 求解本质矩阵 E
 *        - recoverPose() 通过本质矩阵得到 R t
 * @param[in] corres 对应归一化坐标点对
 * @param[out] Rotation 当前帧到参考帧(l 帧)的旋转矩阵
 * @param[out] Translation 当前帧到参考帧(l 帧)的平移向量
 * @return true -- 内点数大于 12，否则视为求解失败
 */
bool MotionEstimator::solveRelativeRT(
  ...
  if (corres.size() >= 15) {
     ...
     *  InputArray  points1, 第一幅图像点的数组(归一化坐标)
     *  InputArray  points2, 第二幅图像点的数组(归一化坐标)
     *  int method = FM_RANSAC, RANSAC 算法
     *  计算fundamentalmatrix的方法，这里使用了cv::FM_RANSAC，说明使用了8点法进行求解的
     ...
     *  OutputArray mask = noArray() 默认在计算过程中没有被舍弃的点
     *  )
     */
    cv::Mat E =
