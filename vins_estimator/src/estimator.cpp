...
/**
 * @brief 处理IMU数据，进行IMU预积分
 *        通过中值积分的离散形式得到PQV
 * @param[in] dt imu 数据帧之间的时间间隔
...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration,
  ...
  if (frame_count != 0) {
    // 真正是在 push_back 中的 propagate() -> midPointIntegration()进行预积分
...
void Estimator::processImage(
  ...
  // all_image_frame 用来做初始化相关操作，保留滑窗起始到当前的所有帧
  // 有一些帧会因为不是KF，被MARGIN_SECOND_NEW，但是即使较新的帧被margin，他也会保留在这个容器中
  // !因为初始化要求使用所有的帧，而非只要KF
  ImageFrame imageframe(image, header.stamp.toSec());
  imageframe.pre_integration = tmp_pre_integration;
  // 这里就是简单的把图像和预积分绑定在一起，这里预积分就是两图像帧之间的，滑窗中实际上是两个KF之间的
  // 实际上是准备用来初始化的相关数据
  all_image_frame.insert(make_pair(header.stamp.toSec(), imageframe));
  // tmp_pre_integration 重新初始化
  tmp_pre_integration =
      new IntegrationBase{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count]};

  // 外参初始化，对imu和相机之间的外参进行初步估计
  // 外参标定指的是对相机坐标系到imu坐标系的变换矩阵进行在线标定和优化
  ...
  // INITIAL 表示初始化未完成 -- 待初始化
  // 单目紧耦合的 VIO 是个高度非线性的系统，在系统开始的时候需要准确的初始化估计
  // 初始化分为两个部分
  // -- 利用 SFM 进行纯视觉估计滑窗内所有帧的位姿以及所有 3D 点的逆深度
  // -- 与 IMU 预积分进行对齐求解初始化参数
  //    - 重力向量 g
  //    - 单目的尺度因子
  //    - 每一帧对应的速度
  //    - 陀螺仪 bias
  if (solver_flag == INITIAL) {
    if (frame_count == WINDOW_SIZE) {
      bool result = false;
      // 标定外参成功且当前帧距离上次(尝试)初始化时间戳大于0.1s，就进行初始化操作
      if (ESTIMATE_EXTRINSIC != 2 &&
...
bool Estimator::initialStructure() {
  // check imu observibility -- 通过加速度标准差判断imu是否有充分运动
  // ? return false 都没用...
  {
  ...
  // sfm 重建出的 3d landmarks 坐标
  map<int, Vector3d> sfm_tracked_points;
  vector<SFMFeature> sfm_f;

  // 将 f_manager.feature 中的 feature 存储到 sfm_f 中
  ...
  Vector3d relative_T;
  int l;
  // 返回窗口第一个满足与最新帧视差要求的帧l,以及最新帧到l帧的R t
  ...
  // 对窗口中每个图像帧求解sfm
  // 得到所有关键帧相对于参考帧l的旋转四元数Q,平移向量T和landmarks坐标sfm_tracked_points
  GlobalSFM sfm;
  ...
  // 对于所有的图像帧，包括不在滑动窗口内的非关键帧，提供初始的 R,t 估计
  // 然后 solve PnP 进行优化，得到每一帧在imu坐标系下的位姿--body 坐标系
  map<double, ImageFrame>::iterator frame_it;
  ...
  for (int i = 0; frame_it != all_image_frame.end(); frame_it++) {
    ...
    // 下面这两个式子构造很巧妙，为什这么做参考visualInitialAlign中Ps[]的计算部分
    // 求解出 b_k -> c_l 的旋转矩阵--R_bk^cl
...
bool Estimator::visualInitialAlign() {
  ...
  // change state
  // 获取滑动窗口内所有关键帧相对于 l 帧的位姿，设置为关键帧
  for (int i = 0; i <= frame_count; i++) {
    Matrix3d Ri = all_image_frame[Headers[i].stamp.toSec()].R;
    Vector3d Pi = all_image_frame[Headers[i].stamp.toSec()].T;

    // 此时 Ps[] Rs[] 还是相对于 l 帧的位姿
    Ps[i] = Pi;
    Rs[i] = Ri;
  ...
  // 此时陀螺仪 bias 发生改变，对滑窗中的预积分重新进行计算
  for (int i = 0; i <= WINDOW_SIZE; i++) {
    pre_integrations[i]->repropagate(Vector3d::Zero(), Bgs[i]);
  }

  /**
   * 前面初始化中，计算出来的是ck相对滑动窗口中l的位姿，这里转化为bk到b0
   *
   * t_{bk}^{b0} = t_{bk}^{c0} - t_{b0}^{c0}
   *
   * NOTICE 如果配置文件中无外参，这里的 TIC[0] 都是 0
   */
  ...
  // 求出的g是第l帧相机坐标系下的重力方向
  /**
   * 重力细化后得到的是l坐标系下的重力，这里通过将 g^{c_0} 转至 z(yaw) 轴方向
   * 调整到世界坐标系中，可以计算相机参考帧到惯性世界坐标系的旋转矩阵 q_{c_0}^w
   */
  // ? 这里没懂，直接看求出的结果...
  ...
  // rot_diff 等价于 q_{c_0}^w--到世界坐标系的旋转矩阵
...
/**
 * @brief find previous frame which contians enough correspondance and parallex
 *        with newest frame(对应滑窗中最后一帧)
 * @param[out] relative_R 最新帧到第 l 帧之间的旋转矩阵 R
 * @param[out] relative_T 最新帧到第 l 帧之间的平移向量 T
 * @param[out] l 第一个满足要求的帧，这里的 l 帧作为参考帧做全局 sfm 用
 * @return true--可以进行初始化 false--不满足条件
 */
  ...
  for (int i = 0; i < WINDOW_SIZE; i++) {
    ...
    if (corres.size() > 20) {
      ...
      for (int j = 0; j < int(corres.size()); j++) {
        ...
        // 计算 vector 计算 l2 norm 表示视差
        double parallax = (pts_0 - pts_1).norm();
        sum_parallax = sum_parallax + parallax;
      }
      average_parallax = 1.0 * sum_parallax / int(corres.size());
      if (average_parallax * 460 > 30 &&
          // solveRelativRT() 通过计算最新一帧到l帧的
          // R,T,并判断RANSAC内点是否足够(>12)
...
void Estimator::optimization() {
  ...
  // 添加优化变量参数块
  // 添加优化变量--相机位姿，速度，加速度偏差，陀螺仪偏差
  for (int i = 0; i < WINDOW_SIZE + 1; i++) {
    ceres::LocalParameterization *local_parameterization =
        new PoseLocalParameterization();
    // para_pose[i] 中存放的是滑动窗口中关键帧的位姿
    // para_Pose[i] 的大小为 SIZE_POSE，值为 7
    problem.AddParameterBlock(para_Pose[i], SIZE_POSE, local_parameterization);
    // SIZE_SPEEDBIAS 设置为 9
    // 速度(3)，加速度计Bias(3)，陀螺仪Bias(3)
  ...
  // 将要优化的变量转化为 double 数组格式
  // 以满足 AddParameterBlock 的要求
  vector2double();

  // 添加边缘化的先验残差信息，第一次进行优化时 last_marginalization_info 为空
  ...
  for (auto &it_per_id : f_manager.feature) {
    ...
    for (auto &it_per_frame : it_per_id.feature_per_frame) {
      ...
      if (ESTIMATE_TD) {
        ...
        // 约束的变量是该特征点的第一个观测帧以及找出的观测帧，加上外参和特征点逆深度
  ...
  // 设定优化使用的算法，这里使用置信域类优化算法 dogleg
  options.trust_region_strategy_type = ceres::DOGLEG;
  // 设置迭代求解的次数，定义在配置文件中
  options.max_num_iterations = NUM_ITERATIONS;
  if (marginalization_flag == MARGIN_OLD)
    // 边缘化操作中边缘旧帧的操作比较多，为了保证速度给的优化时间少
