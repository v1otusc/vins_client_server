...
void registerPub(ros::NodeHandle &n) {
  ...
  pub_odom_pose =
      n.advertise<geometry_msgs::TransformStamped>("transform", 1000);
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header) {
  ...
  transform.setRotation(q);
  br.sendTransform(tf::StampedTransform(transform, header.stamp, IMU_FRAME_NAME,
                                        CAMERA_FRAME_NAME));
...
void pubKeyframe(const Estimator &estimator) {
  ...
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR &&
    ...
    for (auto &it_per_id : estimator.f_manager.feature) {
      ...
      if (it_per_id.start_frame < i &&
          it_per_id.start_frame + frame_size - 1 >= i &&
          it_per_id.solve_flag == 1) {
        int imu_i = it_per_id.start_frame;
        Vector3d pts_i =
            it_per_id.feature_per_frame[0].point * it_per_id.estimated_depth;
        ...
        Eigen::Vector3d pts_imu_i = R_mat.transpose() * (w_pts_i - P);
        Eigen::Vector3d pts_cam_i =
            estimator.ric[0].transpose() * (pts_imu_i - estimator.tic[0]);
        p.x = pts_cam_i(0);
        p.y = pts_cam_i(1);
        p.z = pts_cam_i(2);
        point_cloud.points.push_back(p);

        int imu_j = WINDOW_SIZE - 2 - skipped_keyframes - it_per_id.start_frame;
