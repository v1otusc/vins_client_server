...
void process() {
  while (true) {
    ...
    for (auto &measurement : measurements) {
      ...
      TicToc t_s;
      // 建立每个特征点的 map 为 (camera_id(单目==0) - [x,y,z,u,v,vx,vy])
      // 索引为feature_id
...
int main(int argc, char **argv) {
  ...
  // resolveName, eg. "blah" => "/namespace/blah"
  ros::Subscriber sub_image =
      n.subscribe(n.resolveName("features"), 2000, feature_callback);
  ros::Subscriber sub_restart =
      n.subscribe(n.resolveName("feature_restart"), 2000, restart_callback);
